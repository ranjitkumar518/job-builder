...
if [ $env == DEV ]; then
   ...
   if [ $jobtype == Gsf ]; then
      ...
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/GSF_DEV/variables.yaml
      ...
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/NONGSF_DEV/
...
if [ $env == UAT ]; then
   ...
   if [ $jobtype == Gsf ]; then
      ...
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/GSF_UAT/variables.yaml
      echo "Job builder is about to create a Gsf UAT job "
      $jjb_home update $user_home/GSF_UAT/
   elif [ $jobtype == Nongsf ]; then
      echo "This is a Non GSF UAT Job"
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/jobs/CI_Jobs/NONGSF_UAT/
...
if [ $env == IT ]; then
   ...
   if [ $jobtype == Gsf ]; then
      ...
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/GSF_IT/
      echo "Job builder is about to create a Gsf IT job "
      $jjb_home update $user_home/GSF_IT/
   elif [ $jobtype == Nongsf ]; then
      echo "This is a Non GSF IT Job"
      sudo mv $workspace/$JOB_NAME/variables.yaml $user_home/NONGSF_IT/
